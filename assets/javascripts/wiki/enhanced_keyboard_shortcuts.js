...
window.addEventListener("DOMContentLoaded", () => {
  ...
  $("#main").on("keydown", "textarea", (e) => {
    ...
    // Detect desktop Mac using userAgent (excluding mobile iOS devices)
    const isMac =
    ...
    if (ctrlOrCmd && e.key === "x" && !e.shiftKey && !e.altKey) {
      ...
      if (start === end) {
        ...
        // Remove the line
        const newValue =
          info.text.substring(0, info.lineStart) +
          info.text.substring(
            info.lineEnd + (info.lineEnd < info.text.length ? 1 : 0),
          );
    ...
    if (e.altKey && e.key === "ArrowUp" && !ctrlOrCmd && !e.shiftKey) {
      ...
      // Swap the lines
      const newValue =
        info.text.substring(0, prevLineStart) +
        info.linesText +
        (hasTrailingNewline ? "\n" : "") +
        prevLineText +
        "\n" +
    ...
    if (e.altKey && e.key === "ArrowDown" && !ctrlOrCmd && !e.shiftKey) {
      ...
      const actualNextLineEnd =
        nextLineEnd === -1 ? info.text.length : nextLineEnd;
      const nextLineText = info.text.substring(
        nextLineStart,
        actualNextLineEnd,
      );
      ...
      const afterLines = nextLineHasNewline
        ? info.text.substring(actualNextLineEnd)
        : "";
    ...
    if (e.shiftKey && e.altKey && e.key === "ArrowDown" && !ctrlOrCmd) {
      ...
      const newValue =
        info.text.substring(0, info.lineEnd) +
        "\n" +
        info.linesText +
